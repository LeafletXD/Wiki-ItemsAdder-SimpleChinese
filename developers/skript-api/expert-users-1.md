...
{% hint style="info" %}
有关 **skript-reflect** 的更多信息，请阅读其 [wiki](https://tpgamesnl.gitbook.io/skript-reflect/)
{% endhint %}

{% hint style="danger" %}
#### 请**不要询问**有关**skript**的**支持**或**问题**。

我不是 skript 专家，也不是 **skript** 或 **skript-reflect** 的开发者。\
**有关 skript 的所有问题将被忽略**，希望你能理解。
{% endhint %}
...
on PlayerInteractEvent:
    ...
    event.getPlayer().sendMessage("是否为自定义方块: %{_isCustomBlock}%")
...
function item_menu(player: player, pag: number):
    ...
    loop {items::*}:
        ...
        if {_total} = 45:
            add 1 to {_pags}
            set {_total} to 0
    set {_inv} to chest with 6 row named "&6页 &l%{_pag}%/%{_pags}%"
    wait tick
    ...
    if (amount of {items::*}) > {_pagina_inicio} + 45:
        set slot 53 of {_inv} to arrow named "&6下一页 >>"
    else:
        fillInv({_inv}, 53)
    if {_pag} > 0:
        set slot 45 of {_inv} to arrow named "&6<< 上一页"
    else:
        fillInv({_inv}, 45)
...
on inventory click:
    if event-inventory = (metadata value "item_menu" of player):
        ...
        if event-slot = 53:
            if event-slot is arrow named "&6下一页 >>":
                item_menu(player, (metadata "item_menu_pag" of player) + 1)
        if event-slot = 45:
            if event-slot is arrow named "&6<< 上一页":
                item_menu(player, (metadata "item_menu_pag" of player) - 1)
